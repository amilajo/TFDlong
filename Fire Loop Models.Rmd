...
beta.fire <- list(fbeta1[[2]][1]$plot, fbeta2[[2]][1]$plot)
...
```{r check PFOSA models with outliers removed, results = "asis", fig.width = 8, fig.height = 11}

#PFOSA: total.duration.hrs
model_PFOSA <- lm(PFOSAorig ~  total.duration.hrs + Ethnicity, data = FireChange2)
model_PFOSA.rm <- lm(PFOSAorig ~  total.duration.hrs + Ethnicity, data = FireChange2 %>% 
                       .[-33, ])

PFOSA.resid<- ggplot(data.frame(resid = resid(model_PFOSA), 
                                 fitted = fitted(model_PFOSA)), 
                                aes(x = fitted, y = resid)) +
  geom_point(size = 2) +
  geom_hline(yintercept = 0) +
  theme(axis.text = element_text(size = 10),
        plot.title = element_text(hjust = 0.5)) +  
  labs(title = "PFOSAorig: total duration of fireruns (hrs)")
PFOSA.resid.rm <- ggplot(data.frame(resid = resid(model_PFOSA.rm), 
                                     fitted = fitted(model_PFOSA.rm)), 
                          aes(x = fitted, y = resid)) +
  geom_point(size = 2) +
  geom_hline(yintercept = 0) +
  theme(axis.text = element_text(size = 10),
        plot.title = element_text(hjust = 0.5)) +
  labs(title = "Outliers removed [-33, ]")

plotlist.PFOSA <- list(PFOSA.resid, PFOSA.resid.rm)
print(ggarrange(plotlist = plotlist.PFOSA, nrow = 2, ncol = 2, font.label = list(size = 10)))

stargazer(model_PFOSA, model_PFOSA.rm, title = "PFOSA: total duration of fireruns (hours) - Change Score Models 
          - 1) original and 2) outliers removed",
          single.row = FALSE, header = FALSE, omit.stat = c("f", "ser"), report = "vctp*",
          intercept.bottom = FALSE, intercept.top = TRUE,
          digits = 3, font.size = "scriptsize")

#PFOSA: days.since.MRF
model_PFOSA2 <- lm(PFOSAorig ~  days.since.MRF + Ethnicity, data = FireChange2)
model_PFOSA2.rm <- lm(PFOSAorig ~  days.since.MRF + Ethnicity, data = FireChange2 %>% 
                       .[-33, ] %>%
                       .[-25, ])

PFOSA.resid2<- ggplot(data.frame(resid = resid(model_PFOSA2), 
                                 fitted = fitted(model_PFOSA2)), 
                                aes(x = fitted, y = resid)) +
  geom_point(size = 2) +
  geom_hline(yintercept = 0) +
  theme(axis.text = element_text(size = 10),
        plot.title = element_text(hjust = 0.5)) +  
  labs(title = "PFOSAorig: days since most recent fire")
PFOSA.resid2.rm <- ggplot(data.frame(resid = resid(model_PFOSA2.rm), 
                                     fitted = fitted(model_PFOSA2.rm)), 
                          aes(x = fitted, y = resid)) +
  geom_point(size = 2) +
  geom_hline(yintercept = 0) +
  theme(axis.text = element_text(size = 10),
        plot.title = element_text(hjust = 0.5)) +
  labs(title = "Outliers removed [-33, ], [-25, ]")

plotlist.PFOSA2 <- list(PFOSA.resid2, PFOSA.resid2.rm)
print(ggarrange(plotlist = plotlist.PFOSA2, nrow = 2, ncol = 2, font.label = list(size = 10)))

stargazer(model_PFOSA2, model_PFOSA2.rm, title = "PFOSA: days since most recent fire - Change Score Models 
          - 1) original and 2) outliers removed",
          single.row = FALSE, header = FALSE, omit.stat = c("f", "ser"), report = "vctp*",
          intercept.bottom = FALSE, intercept.top = TRUE,
          digits = 3, font.size = "scriptsize")

#PFOSA: days.since.struc.MRF
model_PFOSA3 <- lm(PFOSAorig ~  days.since.struc.MRF + Ethnicity, data = FireChange2)
model_PFOSA3.rm <- lm(PFOSAorig ~  days.since.struc.MRF + Ethnicity, data = FireChange2 %>% 
                       .[-33, ] %>%
                       .[-25, ])

PFOSA.resid3<- ggplot(data.frame(resid = resid(model_PFOSA3), 
                                 fitted = fitted(model_PFOSA3)), 
                                aes(x = fitted, y = resid)) +
  geom_point(size = 2) +
  geom_hline(yintercept = 0) +
  theme(axis.text = element_text(size = 10),
        plot.title = element_text(hjust = 0.5)) +  
  labs(title = "PFOSAorig: days since most recent structural fire")
PFOSA.resid3.rm <- ggplot(data.frame(resid = resid(model_PFOSA3.rm), 
                                     fitted = fitted(model_PFOSA3.rm)), 
                          aes(x = fitted, y = resid)) +
  geom_point(size = 2) +
  geom_hline(yintercept = 0) +
  theme(axis.text = element_text(size = 10),
...
plotlist.PFOSA3 <- list(PFOSA.resid3, PFOSA.resid3.rm)
print(ggarrange(plotlist = plotlist.PFOSA3, nrow = 2, ncol = 2, font.label = list(size = 10)))

stargazer(model_PFOSA3, model_PFOSA3.rm, title = "PFOSA: days since most recent structural fire - Change Score Models 
          - 1) original and 2) outliers removed",
          single.row = FALSE, header = FALSE, omit.stat = c("f", "ser"), report = "vctp*",
          intercept.bottom = FALSE, intercept.top = TRUE,
          digits = 3, font.size = "scriptsize")

#PFOSA: time.since.baseline
model_PFOSA4 <- lm(PFOSAorig ~  time.since.baseline + Ethnicity, data = FireChange2)
model_PFOSA4.rm <- lm(PFOSAorig ~  time.since.baseline + Ethnicity, data = FireChange2 %>% 
                       .[-33, ] %>%
                       .[-25, ])

PFOSA.resid4<- ggplot(data.frame(resid = resid(model_PFOSA4), 
                                 fitted = fitted(model_PFOSA4)), 
                                aes(x = fitted, y = resid)) +
  geom_point(size = 2) +
  geom_hline(yintercept = 0) +
  theme(axis.text = element_text(size = 10),
        plot.title = element_text(hjust = 0.5)) +  
  labs(title = "PFOSAorig: time since baseline (days)")
PFOSA.resid4.rm <- ggplot(data.frame(resid = resid(model_PFOSA4.rm), 
                                     fitted = fitted(model_PFOSA4.rm)), 
                          aes(x = fitted, y = resid)) +
  geom_point(size = 2) +
  geom_hline(yintercept = 0) +
  theme(axis.text = element_text(size = 10),
...
plotlist.PFOSA4 <- list(PFOSA.resid4, PFOSA.resid4.rm)
print(ggarrange(plotlist = plotlist.PFOSA4, nrow = 2, ncol = 2, font.label = list(size = 10)))

stargazer(model_PFOSA4, model_PFOSA4.rm, title = "PFOSA: time since baseline (days) - Change Score Models 
          - 1) original and 2) outliers removed",
          single.row = FALSE, header = FALSE, omit.stat = c("f", "ser"), report = "vctp*",
          intercept.bottom = FALSE, intercept.top = TRUE,
          digits = 3, font.size = "scriptsize")
...
```{r check nPFOA models with outliers removed, results = 'asis', fig.width = 8, fig.height = 11}

#nPFOA: total.num.fireruns 
model_nPFOA <- lm(nPFOAorig ~  days.since.struc.MRF + Ethnicity, data = FireChange2)
model_nPFOA.rm <- lm(nPFOAorig ~  days.since.struc.MRF + Ethnicity, data = FireChange2 %>%
                       .[-40, ] %>%
                       .[-4, ])
...
plotlist.nPFOA1 <- list(nPFOA.resid, nPFOA.resid.rm)
print(ggarrange(plotlist = plotlist.nPFOA1, nrow = 2, ncol = 2, font.label = list(size = 10)))

stargazer(model_nPFOA, model_nPFOA.rm, title = "nPFOA: Total number of fireruns - Change Score Models 
          - 1) original and 2) outliers removed",
          single.row = FALSE, header = FALSE, omit.stat = c("f", "ser"), report = "vctp*",
          intercept.bottom = FALSE, intercept.top = TRUE,
          digits = 3, font.size = "scriptsize")
...
plotlist.nPFOA2 <- list(nPFOA.resid2, nPFOA.resid2.rm)
print(ggarrange(plotlist = plotlist.nPFOA2, nrow = 2, ncol = 2, font.label = list(size = 10)))

stargazer(model_nPFOA2, model_nPFOA2.rm, title = "nPFOA: Total number of structural fireruns - Change Score Models 
          - 1) original and 2) outliers removed",
          single.row = FALSE, header = FALSE, omit.stat = c("f", "ser"), report = "vctp*",
          intercept.bottom = FALSE, intercept.top = TRUE,
          digits = 3, font.size = "scriptsize")
...
# summary(model_nPFOA3)
# summary(model_nPFOA3.rm)
# plot(resid(model_nPFOA3) ~ fitted(model_nPFOA3))
# abline(h = 0)
# plot(resid(model_nPFOA3.rm) ~ fitted(model_nPFOA3.rm))
# abline(h=0)
...
plotlist.nPFOA3 <- list(nPFOA.resid3, nPFOA.resid3.rm)
print(ggarrange(plotlist = plotlist.nPFOA3, nrow = 2, ncol = 2, font.label = list(size = 10)))

stargazer(model_nPFOA3, model_nPFOA3.rm, title = "nPFOA: total duration of fireruns (hours) - Change Score Models 
          - 1) original and 2) outliers removed",
          single.row = FALSE, header = FALSE, omit.stat = c("f", "ser"), report = "vctp*",
          intercept.bottom = FALSE, intercept.top = TRUE,
          digits = 3, font.size = "scriptsize")
...
plotlist.nPFOA4 <- list(nPFOA.resid4, nPFOA.resid4.rm)
print(ggarrange(plotlist = plotlist.nPFOA4, nrow = 2, ncol = 2))

stargazer(model_nPFOA4, model_nPFOA4.rm, title = "nPFOA: total duration of structural fireruns (hours) 
...
plotlist.nPFOA5 <- list(nPFOA.resid5, nPFOA.resid5.rm)
print(ggarrange(plotlist = plotlist.nPFOA5, nrow = 2, ncol = 2, font.label = list(size = 10)))

stargazer(model_nPFOA5, model_nPFOA5.rm, title = "nPFOA: days since most recent fire - Change Score Models - 1)
          original and 2) outliers removed",
          single.row = FALSE, header = FALSE, omit.stat = c("f", "ser"), report = "vctp*",
          intercept.bottom = FALSE, intercept.top = TRUE,
          digits = 3, font.size = "scriptsize")
...
plotlist.nPFOA6 <- list(nPFOA.resid6, nPFOA.resid6.rm)
print(ggarrange(plotlist = plotlist.nPFOA6, nrow = 2, ncol = 2, font.label = list(size = 10)))

stargazer(model_nPFOA6, model_nPFOA6.rm, title = "nPFOA: days since most recent structural fire - 
          Change Score Models - 1) original and 2) outliers removed",
          single.row = FALSE, header = FALSE, omit.stat = c("f", "ser"), report = "vctp*",
          intercept.bottom = FALSE, intercept.top = TRUE,
          digits = 3, font.size = "scriptsize")
...
plotlist.nPFOA7 <- list(nPFOA.resid7, nPFOA.resid7.rm)
print(ggarrange(plotlist = plotlist.nPFOA7, nrow = 2, ncol = 2, font.label = list(size = 10)))

stargazer(model_nPFOA7, model_nPFOA7.rm, title = "nPFOA: time since baseline (days) - 
          Change Score Models - 1) original and 2) outliers removed",
          single.row = FALSE, header = FALSE, omit.stat = c("f", "ser"), report = "vctp*",
          intercept.bottom = FALSE, intercept.top = TRUE,
          digits = 3, font.size = "scriptsize")
...
```{r check nPFOS models with outliers removed, results = 'asis', fig.width = 8, fig.height = 11}
...
```{r check PFDEA models with outliers removed, results = 'asis', fig.width = 8, fig.height = 11}
...
```{r check PFHXS models with outliers removed, results = 'asis', fig.width = 8, fig.height = 11}
...
```{r check PFNA models with outliers removed, results = 'asis', fig.width = 8, fig.height = 11}
...
```{r check PFUA models with outliers removed, results = 'asis', fig.width = 8, fig.height = 11}
...
```{r check SbPFOA models with outliers removed, results = 'asis', fig.width = 8, fig.height = 11}
