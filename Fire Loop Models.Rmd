...
cd.table <- data.frame(Original.indices = as.numeric(),
                       Transformed.indices = as.numeric())

cd.table2 <- rbind(as.numeric(cd.plot.orig$problem_indices[1]), as.numeric(cd.plot.trans$problem_indices[1]), 
                   na.rm = FALSE) 

cd.table3 <- rbind(cd.table, cd.table2)  %>%
  mutate(Orig.indices = cd.plot.orig$problem_indices[1],
         Trans.indices = cd.plot.trans$problem_indices[1]) %>%
  dplyr::select(-V1) 

  cd.table4 <- cd.table3[-c(2, 3) , ]
  cd.table5 <- ggtexttable(cd.table4, rows = NULL, theme = ttheme("blank"))
...
fbeta2 <- fbetas_plot(out_info$transmdl, paste0(out_info$transmdl$terms[[2]], ": ",
  ...
  beta1 <- as.numeric(fbeta1$`(Intercept)`$problem_indices)
  beta2 <- as.numeric(fbeta1$`[[2]][1]`$problem_indices)
  beta3 <- as.numeric(fbeta1$EthnicityHispanic$problem_indices)
  beta4 <- as.numeric(fbeta2$`(Intercept)`$problem_indices)
  beta5 <- as.numeric(fbeta2$`[[2]][1]`$problem_indices)
  beta6 <- as.numeric(fbeta2$EthnicityHispanic$problem_indices)
  beta.out <- list(Intercept.indices.orig = as.numeric(beta1[1]),
              Exposure.indices.orig  = as.numeric(beta2[1]),
              EthnHisp.indices.orig = as.numeric(beta3[1]),
              Intercept.indices.trans = as.numeric(beta4[1]),
              Exposure.indices.trans  = as.numeric(beta5[1]),
              EthnHisp.indices.trans = as.numeric(beta6[1]))
  beta.out1 <- as.data.frame(beta.out)

beta.table <- data.frame(Intercept.indices.orig = as.numeric(),
                         Intercept.indices.trans = as.numeric(),
                       Exposure.indices.orig = as.numeric(),
                       Exposure.indices.trans = as.numeric(),
                       EthnHisp.indices.orig = as.numeric(),
                       EthnHisp.indices.trans = as.numeric())


beta.table2 <- rbind(beta.table, beta.out1) %>%
  gather(key = "variable", value = "value") %>%
  rename(indices = value,
         Model.terms = variable)

beta.table3 <- ggtexttable(beta.table2, rows = NULL, theme = ttheme("blank"))


plotlist.a <- list(orig.resid, trans.resid, 
                   qq.plot.orig, qq.plot.trans, 
                   cd.plot.orig$plot,
                   cd.plot.trans$plot, cd.table5)


plotlist.b <- list(fbeta1$`(Intercept)`$plot, fbeta2$`(Intercept)`$plot, 
                   fbeta1$EthnicityHispanic$plot, fbeta2$EthnicityHispanic$plot, 
                   fbeta1[[2]][1]$plot, fbeta2[[2]][1]$plot,
                   beta.table3)

# cat(noquote(paste0("## ", out_info$pfasmdl$terms[[2]], ":", out_info$pfasmdl$terms[[3]][2], " - ",
#                                       " Model Residuals and cooks distance plots and problem indices")))
...
# cat(noquote(paste0("## ", out_info$pfasmdl$terms[[2]], ":", out_info$pfasmdl$terms[[3]][2], " - ",
#                                       " DFBETA plots and problem indices")))
...
# nPFOA: structural number of fireruns and duration of structural fires\
